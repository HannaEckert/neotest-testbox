...
local base = require("neotest-testbox.base")
local collect_results = require("neotest-testbox.results")
...
return function(config)
  ...
  local function discover_positions(path)
    return lib.treesitter.parse_positions(
		path,
		base.treesitter_query,
		{
			require_namespaces = true,
			nested_namespaces = true,
			nested_tests = true,
			position_id = function(position)
				return position.id .. "-" .. position.name .. "-" .. position.path
			end,
		}
	)
  end
  ...
  local TestboxNeotestAdapter = {
    name = "neotest-testbox",
    ...
    build_spec = function(args)
      ...
      -- the local path to the testbox.nvim plugin installed
      ---@type string
      local testbox_dir = vim.fn.fnamemodify(
        debug.getinfo(require("testbox.path").__index).source:match("@?(.*[/\\])"),
        ":p:h:h:h"
      )
      ...
      local command = vim
        .iter({
          ...
          "--noplugin",
          -- add testbox.nvim to &runtimepath (should be available before init config)
          "--cmd",
          ([[lua vim.opt.runtimepath:prepend('%s')]]):format(vim.fn.escape(testbox_dir, " '")),
